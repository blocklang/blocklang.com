...
import * as css from './new-harmonyos-project.m.css';
...
const factory = create({ store, i18n }).properties<NewHarmonyOSProjectProperties>();

export default factory(function NewHarmonyOSProject({ properties, middleware: { store, i18n } }) {
	...
	if (!repositoryResource && !isLoading) {
		executor(initForNewGroupProcess)({ owner, repo: repoName });
	}
	if (!isLoaded) {
		return <Spinner />;
	}
	...
	return (
		<div classes={[c.container]}>
			...
			<div classes={[c.container]} styles={{ maxWidth: '700px' }}>
				<h4>{messages.newHarmonyOSProject}</h4>
				{renderBreadcrumb()}
				<hr />
				{saveFailedErrors && (
