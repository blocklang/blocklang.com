...
export default class ViewProject extends ThemedMixin(I18nMixin(WidgetBase))<ViewProjectProperties> {
	...
	private _renderTable() {
		...
		return v('div', { classes: [c.card] }, [
			// 最近提交信息区
			v('div', { classes: [c.card_header, c.text_muted, c.px_2, c.border_bottom_0, css.recentCommit] }, [
				// 最近提交的用户信息
			...
			v('table', { classes: [c.table, c.table_hover] }, [
		...
		return v('tr', [
			...
			v('td', { classes: [css.icon] }, [
				w(FontAwesomeIcon, {
					...
					title: projectResource.resourceType
				})
			]),
			// 资源名称
			v('td', { classes: [css.content] }, [w(Link, { to: '' }, [`${projectResource.name}`])]),
			// 最近提交信息
	...
	private _renderReadme() {
		...
		return v('div', { classes: [c.card, c.mt_3] }, [
			...
			v('div', { classes: [c.card_body, c.markdown_body] }, [
