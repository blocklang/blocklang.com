...
13. 测试用例使用 assertj
14. 将 github 等第三方网站的 client-id 和 client-security 存到数据库中
15. 使用 SPA 后，怎么控制客户端的权限问题，如怎么控制在用户未登录时，输入 `projects/new` 后不显示创建项目页面
16. client: 将 HashHistory 修改为 StateHistory，但要在服务器端重写获取 js 和 css 文件的方法
17. 使用 github 登录失败后，页面链接变为 `?error`，但仍停留在公共首页，没有提示
18. 编写快速入门文档，发布到百度经验、开源中国等网站 (*)
19. 录制快速入门视频 (*)
20. 第一次发布：完成项目的创建、发布和安装，并编写使用手册和快速入门文档