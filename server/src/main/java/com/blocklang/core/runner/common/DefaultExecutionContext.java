...
public class DefaultExecutionContext implements ExecutionContext{
	...
	public String getGitUrl() {
		var gitUrl = this.getStringValue(ExecutionContext.GIT_URL);
		if(!StringUtils.isBlank(gitUrl)) {
			return gitUrl;
		}
		
		var task = this.getValue(ExecutionContext.PUBLISH_TASK, GitRepoPublishTask.class);
		return task.getGitUrl();
	}
}
